...
import '../../view/screen/alarm/show_alarm_page.dart';
import 'notification.dart'; // widget for the alarm exposure
...
Future<List<CustomAlarm?>> initAlarms() async {
  ...
  // Check if there is saved alarm data
  if (prefs.containsKey('alarmList')) {
    // Decode the JSON-encoded alarm list
    List<String> savedAlarmJson = prefs.getStringList('alarmList') ?? [];
    List<CustomAlarm?> savedAlarmList = savedAlarmJson.map((alarm) {
      return CustomAlarm.fromJson(jsonDecode(alarm));
    }).toList();

    dev.log("Alarms loaded from SharedPreferences: ${savedAlarmList.length}", name: 'Alarm');
    return savedAlarmList;
  } else {
    dev.log("No saved alarms found. Initializing default alarms.", name: 'Alarm');
    return []; // Return an empty list if no alarms are saved
  }
